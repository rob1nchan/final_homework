...
# ====== 全局图表样式（支持中文） ======
sns.set_theme(style="whitegrid")  # 必须先 set_theme，再覆盖字体（否则字体配置会被它重置）
plt.rcParams.update({
...
def plot_top10_zones(df: pd.DataFrame, zone_map: dict) -> None:
    ...
    sns.barplot(x=top_pu.values, y=top_pu.index, ax=ax1, hue=top_pu.index, palette="Blues_r", legend=False)
    ...
    sns.barplot(x=top_do.values, y=top_do.index, ax=ax2, hue=top_do.index, palette="Reds_r", legend=False)
